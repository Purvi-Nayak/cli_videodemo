...
const App = () => {
  ...
  const initializeApp = async () => {
    ...
    try {
      ...
      if (!isConnected) {
        addLog(' Native module not properly connected');
      }
    ...
    if (Platform.OS === 'android') {
      try {
        ...
        if (!allGranted) {
          ...
          addLog(' Storage permissions not granted');
        } else {
  ...
  const pickVideos = async () => {
    try {
      ...
      for (const file of results) {
        ...
        if (!isValid) {
          addLog(` Skipping invalid file: ${file.name}`);
          continue;
        }
  ...
  const removeVideo = async (videoId: string) => {
    try {
      ...
      await updateProgress();
      addLog(' Video removed from queue');
    } catch (error) {
